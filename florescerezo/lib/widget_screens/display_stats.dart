...
class _DetallePartidaState extends State<DetallePartida> {
  ...
  List<Color> colores = [Colors.red,Colors.red,Colors.red,Colors.red];
  List<String> nombres = [" ? "," ? "," ? "," ? ",];
  ...
  void calcularPuntuacionesPartida(){
   ...
   void  ordenar(){
     ...
     colores.insert(0,Colors.pink.shade100); 
    }));
  }
    ...
    return DefaultTabController(
      ...
      appBar: AppBar(
        ...
        icon: Icon(Icons.arrow_back,color: primaryTextColor,)),
          title: const Text('Resultados:',style: TextStyle(color: primaryTextColor),),
          bottom: const TabBar(
            tabs: <Widget>[
              Tab(
                icon: Icon(FontAwesomeIcons.trophy,color: primaryTextColor,),
              ),
              Tab(
               child: Text("1",style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold,color: primaryTextColor),),
              ),
              Tab(
                child: Text("2",style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold,color: primaryTextColor),),
              ),
              Tab(
                child: Text("3",style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold,color: primaryTextColor),),
              ),
            ],
          ),
      ),
                  ...
                  Expanded(
                    child: Container(
                      child: Center(
                        child: Column(
                          children: [
                            Expanded(child:_vista(puntos[0], colores[0], nombres[0], FontAwesomeIcons.trophy)),
      ...
      child: AnimatedContainer(
          child: Row(
            children: [
              ...
              Expanded(child: Container(
                child: Center(
                  child: FittedBox(child: Text(nombre,
                  style: TextStyle(
                                     color: Colors.white,
...
_perdedores(perdedores){
    switch (perdedores) {
      ...
      return Column(
        children: [
          Expanded(child:_vista(puntos[1], colores[1], nombres[1], Icons.cancel_outlined) ),
        ],
      );
      case 2: 
      return Column(
        children: [
          Expanded(child:_vista(puntos[1], colores[1], nombres[1],  Icons.cancel_outlined) ),
          Expanded(child:_vista(puntos[2], colores[2], nombres[2],  Icons.cancel_outlined) )
        ],
      );
      case 3: 
      return Column(
        children: [
          Expanded(child:_vista(puntos[1], colores[1], nombres[1],  Icons.cancel_outlined) ),
          Expanded(child:_vista(puntos[2], colores[2], nombres[2],  Icons.cancel_outlined) ),
          Expanded(child:_vista(puntos[3], colores[3], nombres[3],  Icons.cancel_outlined) )
        ],
      );
    }
  }
